...
| git checkout <分支名> -- <文件路径> | 从其他分支中取出文件，覆盖到当前工作目录，但不切换分支                                                                      |                                 |
| git diff --stat                     | 查看工作区文件修改行数                                                                                                      | --numstat配置:具体行数          |
...
| git merge 分支名                         | 合并分支                                                          |                                  |
| git worktree add <文件夹路径> <跟踪分支> | 给已跟踪的分支新建worktree `git worktree add ../xx xx`            | 会自动创建新文件夹并复制分支内容 |
| git diff master..A \| git apply          | 把A分支先于master分支的变动复制到当前分支工作区，并且不产生commit |                                  |
